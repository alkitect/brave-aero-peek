...
First public tag is recorded in `docs/PUBLISH.md` (`First public tag:`). This product tags **`v` + MV3 `browser-extension/manifest.json` `version`** (exception to the usual 0.1.0 greenfield default). Never copy another alkitect repo’s tag. Use `RC-BEFORE-1.0` in PUBLISH only for an intentional 0.9.x RC. After the first tag, bump MV3 version then CHANGELOG, then tag. Shell `metadata.json` `version` is a separate GNOME integer. Maintainers: `./scripts/ci-check.sh` must pass before tag.
...
- Brave `.deb` version (not Flatpak/Snap)
...
- **Release source (after publish):** this GitHub repository.
- **Until publish:** a private Linux customization tree may still be the daily install path; copy behavior into this tree before the first public tag.
